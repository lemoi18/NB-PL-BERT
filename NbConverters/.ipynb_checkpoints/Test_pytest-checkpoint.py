...
def test_cardinal(setup_normalization):
    ...
    assertEqualWithColor(cardinal.convert("21"), "tjueen", "Cardinal conversion for '21'")
...
def test_decimal(setup_normalization):
    ...
    assertEqualWithColor(decimal.convert("2.718"), "to komma sju en åtte", "Decimal conversion for '2.718'")
...
def test_date(setup_normalization):
    ...
    assertEqualWithColor(date.convert("12/03/2021"), "tolvte mars tjue tjueen", "Date conversion for '12/03/2021'")
    assertEqualWithColor(date.convert("01/01/2020"), "første januar tjuetjue", "Date conversion for '01/01/2020'")
    assertEqualWithColor(date.convert("25/12/2022"), "tjuefemte desember tjue tjue to", "Date conversion for '25/12/2022'")
    assertEqualWithColor(date.convert("31/10/1999"), "trettiførste oktober nitten nitti ni", "Date conversion for '31/10/1999'")
...
def test_measure(setup_normalization):
    ...
    assertEqualWithColor(measure.convert("5kg"), "fem kilo", "Measure conversion for '5kg'")
...
def test_telephone(setup_normalization):
    ...
    assertEqualWithColor(telephone.convert("454 90 073"), "fire fem fire ni null null sju tre", "Telephone conversion for '454 90 073'")
    assertEqualWithColor(telephone.convert("555-1234"), "fem fem fem en to tre fire", "Telephone conversion for '555-1234'")
    assertEqualWithColor(telephone.convert("+47 926 51 793"), "plus fire sju ni to seks fem en sju ni tre ", "Telephone conversion for '987-6543'")
...
def test_time(setup_normalization):
    ...
    assertEqualWithColor(time.convert("5:45"), "fem førtifem", "Time conversion for '5:45'")
    assertEqualWithColor(time.convert("00:00"), "null null", "Time conversion for '00:00'")
...
def test_range(setup_normalization):
    ...
    assertEqualWithColor(range_converter.convert("5-7"), "fem til sju", "Range conversion for '5-7'")
...
def test_legal_symbols(setup_normalization):
    ...
    assertEqualWithColor(setup_normalization["legal_symbols"].convert("§ 5 - 7"), "paragraf fem til paragraf sju", "Legal symbols conversion for '§ 5 - 7'")
...
def test_money(setup_normalization):
    ...
    assertEqualWithColor(setup_normalization["money"].convert("kr 100"), "hundre kroner", "Money conversion for 'kr 100'")
    assertEqualWithColor(setup_normalization["money"].convert("NOK 750,000"), "sju hundre femti tusen norske kroner", "Money conversion for 'NOK 750,000'")
